...
function Tree(x1, y1, seedSize, color, blossom, blossomColor){
    ...
    if (!(this.size < 4)){
      ...
      this.contents.push(
        ...
        new Branch(
          ...
          size/1.2 - random(1.5, 1.7),
          length/random(1, 1.2),
          [color[0] + random(0, 10), color[1] + random(0, 10), color[2] + random(0, 5)]
        ),
      ...
      this.contents.push(new Leaf("mid", this.relativeAngle+random(-45,45), random(5,12), [rand,rand+50,rand-20]))
      this.contents.push(new Leaf("end", this.relativeAngle+random(-45,45), random(4,12), [rand,rand+50,rand-20]))
      this.contents.push(new Leaf("end", this.relativeAngle+random(-45,45), random(3,12), [rand,rand+50,rand-20]))
      this.contents.push(new Leaf("mid", this.relativeAngle+random(-45,45), random(2,12), [rand,rand+50,rand-20]))
  ...
  this.renderTree = function(){
    ...
    fill(0, 0, 0, 50);
    noStroke();
    ellipse(this.basex, this.basey+10, Math.pow(this.seed, 3)*10, Math.pow(this.seed, 3));

    fill(60, 50, 40);
    rectMode(RADIUS);
    rect(this.basex, this.basey, Math.pow(this.seed, 3)/2, Math.pow(this.seed, 3)/2, 5);
...
function setup() {
  ...
  tree = new Tree(200, 950, 2.8, [50, 40, 30], false, [180, 200, 240])

  tree2 = new Tree(650, 900, 3, [40, 30, 20])

  tree3 = new Tree(1200, 1025, 3.3, [70, 50, 40], false, [240, 200, 240])
  
  tree4 = new Tree(450, 1000, 2.9, [60, 50, 40], true, [240, 190, 205])
